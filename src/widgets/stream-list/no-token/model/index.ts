...
sample({
  ...
  filter: ({ params }) => params.sorting_filter === 'no_token_stream',
...
sample({
  clock: loadNextPage,
  source: $currentPage,
  fn: (offset) => ({ offset, limit, sorting_filter: 'no_token_stream' as const }),
  target: api.queries.streams.list.refresh,
});

sample({
  clock: onLoadedFirst,
  fn: () => ({ offset: 0, limit, sorting_filter: 'no_token_stream' as const }),
  target: api.queries.streams.list.refresh,
});
