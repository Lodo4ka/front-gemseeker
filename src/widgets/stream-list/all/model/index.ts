...
sample({
  ...
  filter: ({ params }) => params.sorting_filter === 'all_streams',
...
sample({
  clock: loadNextPage,
  source: $currentPage,
  fn: (offset) => ({ offset, limit, sorting_filter: 'all_streams' as const }),
  target: api.queries.streams.list.refresh,
});

sample({
  clock: onLoadedFirst,
  fn: () => ({ offset: 0, limit, sorting_filter: 'all_streams' as const }),
  target: api.queries.streams.list.refresh,
});
