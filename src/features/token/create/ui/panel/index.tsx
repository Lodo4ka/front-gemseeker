...
import { openedModal } from '../../model/modal';

import { $isTokenCreationAllowed } from 'entities/stream';

export const TokenPanel = () => {
  const isTokenCreationAllowed = useUnit($isTokenCreationAllowed);
  const openModal = useUnit(openedModal);

  if (!isTokenCreationAllowed) return null;
    ...
    <div className="bg-darkGray-1 relative flex flex-col gap-6 overflow-hidden rounded-xl p-4">
      ...
      <div className="relative z-2 flex w-full flex-col items-center gap-3">
        <div className="bg-darkGray-1 relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border-[0.5px] border-[#473E24]">
          ...
          <Icon name="coin" size={28} className="text-yellow relative z-2" />
        </div>
        <Typography size="headline3">Launch token</Typography>
        <Typography size="subheadline2" weight="regular" color="secondary">
          Create your own unique token and increase <br /> its market capitalization.
        </Typography>
      </div>
      <Button onClick={openModal} className={{ button: 'relative z-2' }}>
        Launch token
      </Button>
    </div>
  );
