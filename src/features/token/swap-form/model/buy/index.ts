...
sample({
  ...
  filter: ({ 
    ...
    console.log(((token?.real_tokens ?? 0) / LAMPORTS_PER_SOL), exactBtoA({
      value: +value,
      virtualReserv: {
    ...
    return (isEnoughBalanceBuy && ((token?.real_tokens ?? 0) / LAMPORTS_PER_SOL) <= exactBtoA({
      value: +value,
      virtualReserv: {
        token: BigInt(token?.virtual_tokens ?? 0),
        sol: BigInt(token?.virtual_sol ?? 0),
...
sample({
  ...
  filter: ({ 
    ...
    token, 
    exact_amount_in 
  }) => isEnoughBalanceBuy && ((token?.real_tokens ?? 0) / LAMPORTS_PER_SOL) > exactBtoA({
