...
export const rename = createJsonMutation({
  params: declareParams<{ id: number; new_name: string }>(),
  request: {
    method: 'POST',
    url: baseUrl('/wallets/rename'),
    credentials: 'include',
    query: ({ id, new_name }) => ({ wallet_id: id, new_name }),
  },
  response: {
...
applyBarrier(rename, { barrier: barriers.auth });
